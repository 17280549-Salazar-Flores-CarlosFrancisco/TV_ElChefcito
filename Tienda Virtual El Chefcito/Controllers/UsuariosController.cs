...
namespace Tienda_Virtual_El_Chefcito.Controllers
{
    public class UsuariosController : Controller
    {
        ...
        public ActionResult Create(string nombre, string apellidos, string email, string telefono, string rol, int estatus)
        {
            Usuarios user = new Usuarios();
            int id = 0;

            if (!(db.Usuarios.Max(c => (int?)c.id_usuario) == null))
            {
                id = db.Usuarios.Max(c => c.id_usuario);
            }
            else
            {
                id = 0;
            ...
            user.email = Session["correo"].ToString();
            ...
            db.SaveChanges();
            string[] nombres = user.nombre.ToString().Split(' ');
